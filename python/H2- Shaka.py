...
def onValueChange(channel, sampleIndex, val, prev):
    # Get CHOP references
    ok_chop = op('ok_sign_detected')  # Output CHOP for detection
    hand_chop = op('ok_sign')  # CHOP containing hand tracking data

    # Ensure `hand_chop` exists and contains data
    if not hand_chop or hand_chop.numChans == 0:
        return  # Skip processing if no valid hand data

    try:
        # Get Y values for detecting finger positions
        index_tip_x = float(hand_chop['h1:index_finger_tip:x'][0])
        index_tip_y = float(hand_chop['h1:index_finger_tip:y'][0])
        thumb_tip_x = float(hand_chop['h1:thumb_tip:x'][0])
        thumb_tip_y = float(hand_chop['h1:thumb_tip:y'][0])

        middle_tip_y = float(hand_chop['h1:middle_finger_tip:y'][0])
        middle_dip_y = float(hand_chop['h1:middle_finger_dip:y'][0])
        ring_tip_y = float(hand_chop['h1:ring_finger_tip:y'][0])
        ring_dip_y = float(hand_chop['h1:ring_finger_dip:y'][0])
        pinky_tip_y = float(hand_chop['h1:pinky_tip:y'][0])
        pinky_dip_y = float(hand_chop['h1:pinky_dip:y'][0])

    except KeyError:
        return  # Skip processing if channels are missing
    ...
    is_ok_sign = thumb_index_touching and middle_up and ring_up and pinky_up

    # Update the CHOP output
    if ok_chop:
        ok_chop.par.value0 = 1 if is_ok_sign else 0
