def onValueChange(channel, sampleIndex, val, prev):
    finger_chop = op('h2_crossing_fingers')  # Ensure this matches your CHOP name
    detected_chop = op('h2_crossing_fingers_detected')  # Constant CHOP for output

    if detected_chop is None:
        return  # Skip execution if CHOP is missing

    # Read finger positions for the second hand (h2)
    index_tip_x = finger_chop['h2:index_finger_tip:x'][0]
    index_tip_y = finger_chop['h2:index_finger_tip:y'][0]
    middle_tip_x = finger_chop['h2:middle_finger_tip:x'][0]
    middle_tip_y = finger_chop['h2:middle_finger_tip:y'][0]
    ring_tip_y = finger_chop['h2:ring_finger_tip:y'][0]
    pinky_tip_y = finger_chop['h2:pinky_tip:y'][0]
    thumb_tip_y = finger_chop['h2:thumb_tip:y'][0]  # New thumb value

    # **🔧 Adjust these threshold values for better detection**
    UP_THRESHOLD = 0.25  # Fingers "up" if Y is above this value
